...
** TODO Update manual, i.e. explaining that on ffits mode the QSSPC iron calc uses fits, -666 trick, etc.
** TODO Fix end of fit plotted range so always equal length and maximum of points
...
** TODO Add least square value to fittinog options = must eget variance of matrix
...
** TODO Extrapolate the fits to zero and edges always
** DONE Center text
** TODO Do backward calculation from doping concentration to resistivity
** TODO Try to Get COP from fits
...
** TODO Add lifetime values to ascii save and header with fitting parameters and formula and the fit values
** DONE Remove save current plot to PNG file box
** TODO Remove mean of last 10 and instead allow the values to be given for a range
** TODO Make the default generation level 1.92E17
** DONE GIve the percentage of datapoints where delta (deltaN) is exceeded by a value that may be given
** TODO Color points that exceed this value on the maps - give them another unique value somehow?
** TODO Add option to save lifetime maps after image matching
** TODO Change mean of last 10 C values to mean of some deltan range given
