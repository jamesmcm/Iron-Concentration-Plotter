...
* Critical
** TODO FIX MEAN RANGE STUFF - PERHAPS USE FIT?
** TODO FIX FINAL DELTAN calculation for individual C factors. is mean best?
** TODO Fix ValueError: Data has no positive values, and therefore can not be log-scaled. when mean wrong
** TODO Plot of differences (ddn)
** TODO Standard Deviation of Mean [Fei]s
** TODO Comparison of PL maps
** TODO Iron map labels bigger with pixels label
** TODO On QSSPC lifetime plots label the fits
** TODO Caclulate Et from fit data
** TODO Backward calculation Doping conc. to resistivity
** TODO Fix zero lifetime values i.e. make negative again after use - since already have indices needed
** TODO Print percentage of invalid data points, mean of differences, etc. in plot options as well
